...
$$
\text{sim}(q, d_i) = 1 - \cos(\mathbf{e}_q, \mathbf{e}_{d_i}),
$$

где $ \mathbf{e}_q, \mathbf{e}_{d_i} \in \mathbb{R}^{1536} $ — эмбеддинги.
...
- **Перекрытие фактов (fact overlap)** — доля совпадающих категорий фактов между ответом и эталоном:
  $$
  \text{fact\_sim} = \frac{|\{ \text{dates, numbers, names, quoted} \} \cap \text{совпавших}|}{|\{ \text{dates, numbers, names, quoted} \} \cap \text{непустых}|}.
  $$
...
| Метрика | Значение |
|---------|----------|
| Retrieval Hit@5 | **93.3%** (14/15) |
| End-to-End Accuracy | **86.7%** (13/15) |
